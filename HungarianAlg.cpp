...
void AssignmentProblemSolver::assignmentoptimal(int *assignment, double *cost, double *distMatrixIn, int nOfRows, int nOfColumns)
{
	...
	*cost = 0;
	...
	// Memory allocation
	distMatrix    = new double[nOfElements]; //(double *)malloc(nOfElements * sizeof(double));
	memset(distMatrix, 0, nOfElements * sizeof(double));
	...
	// Memory allocation
	coveredColumns = new bool[nOfColumns];// (bool *)calloc(nOfColumns,  sizeof(bool));
	coveredRows    = new bool[nOfRows];// (bool *)calloc(nOfRows,     sizeof(bool));
	starMatrix     = new bool[nOfElements];// (bool *)calloc(nOfElements, sizeof(bool));
	primeMatrix    = new bool[nOfElements];// (bool *)calloc(nOfElements, sizeof(bool));
	newStarMatrix  = new bool[nOfElements];// (bool *)calloc(nOfElements, sizeof(bool)); /* used in step4 */
	...
	delete distMatrix; distMatrix = NULL; //free(distMatrix);
	delete coveredColumns; coveredColumns = NULL; //free(coveredColumns);
	delete coveredRows; coveredRows = NULL; //free(coveredRows);
	delete starMatrix; starMatrix = NULL; //free(starMatrix);
	delete primeMatrix; primeMatrix = NULL; //free(primeMatrix);
	delete newStarMatrix; newStarMatrix = NULL; //free(newStarMatrix);
...
void AssignmentProblemSolver::assignmentsuboptimal1(int *assignment, double *cost, double *distMatrixIn, int nOfRows, int nOfColumns)
{
	...
	*cost = 0;
	...
	/* allocate memory */
	nOfValidObservations  = new int[nOfRows]; //(int *)calloc(nOfRows,    sizeof(int));
	nOfValidTracks        = new int[nOfColumns]; //(int *)calloc(nOfColumns, sizeof(int));
	memset(nOfValidObservations, 0, nOfRows * sizeof(int));
	memset(nOfValidTracks, 0, nOfColumns * sizeof(int));
	...
	/* free allocated memory */
	delete nOfValidObservations; nOfValidObservations = NULL; //free(nOfValidObservations);
	delete nOfValidTracks; nOfValidTracks = NULL; //free(nOfValidTracks);
}
/*
// --------------------------------------------------------------------------
// Usage example
